...
public class PhobosModulesScreen extends TabScreen {
    private static final int ROW = 15, HEADER = 14, TOP = 21;
    private final PhobosGuiTheme look;
    ...
    private record Hit(int x, int y, int width, int height, Panel panel, Module module, Setting<?> setting, String kind) {
        boolean contains(double px, double py) { return PhobosPanelLayout.contains(px, py, x, y, width, height); }
    }
    ...
    public PhobosModulesScreen(PhobosGuiTheme theme) { super(theme, Tabs.get().getFirst()); look = theme; }
    @Override public void initWidgets() {
        if (!panels.isEmpty()) return;
        for (Category category : Modules.loopCategories()) panels.add(new Panel(category, look.getWindowConfig("phobos/" + category.name)));
    }
    @Override protected void init() { super.init(); arrange(false); }
    private void arrange(boolean reset) {
        int w = look.panelWidth.get(), columns = PhobosPanelLayout.columns(width, w);
        int rows = PhobosPanelLayout.bands(panels.size(), columns);
        int band = PhobosPanelLayout.bandHeight(height, rows);
    ...
    @Override public void renderCustom(DrawContext ctx, int mx, int my, float delta) {
        ...
        // Compact utility strip; all Meteor tabs are still reachable with a click.
        int tx = 4;
        for (String action : List.of("Meteor", "HUD", "Reset", "Tabs")) {
        ...
        String help = error.isEmpty() ? "LMB toggle | RMB expand | MMB bind | Shift+RMB full settings | Ctrl+F search" : error;
    ...
    private void drawPanel(DrawContext ctx, Panel panel, int mx, int my) {
        ...
        text(ctx, trim(panel.category.name, w - 15), x + 3, y + 3, 0xffffffff);
        text(ctx, panel.config.expanded ? "-" : "+", x + w - 9, y + 3, 0xffffffff);
        hits.add(new Hit(x, y, w, HEADER, panel, null, null, "header"));
        if (!panel.config.expanded) return;
        List<Module> modules = new ArrayList<>(Modules.get().getGroup(panel.category));
        modules.removeIf(m -> !search.isEmpty() && !(m.title + " " + m.name).toLowerCase(Locale.ROOT).contains(search.toLowerCase(Locale.ROOT)));
        ...
        panel.total = offset;
        int columns = PhobosPanelLayout.columns(width, w);
        int bands = PhobosPanelLayout.bands(panels.size(), columns);
        int cap = PhobosPanelLayout.bandHeight(height, bands) - HEADER - 4;
        panel.view = Math.min(offset, Math.min(cap, Math.max(0, height - y - HEADER - 36)));
        panel.scroll = PhobosPanelLayout.scroll(panel.scroll, offset, panel.view);
        int top = y + HEADER + 2, bottom = top + panel.view;
        ctx.fill(x, y + HEADER, x + w, bottom + 2, 0x77000000);
        ...
        for (Hit row : rows) {
            ...
            ctx.fill(hit.x, ry, hit.x + hit.width, ry + ROW - 1, on ? look.accent(ry, hover) : hover ? 0x88555555 : 0x11555555);
            ...
            String label;
            if (row.kind.equals("module")) label = row.module.title;
            ...
            text(ctx, trim(label, hit.width - (row.kind.equals("module") ? 13 : 4)), hit.x + 2, ry + 3, on || row.setting != null ? 0xffffffff : 0xffaaaaaa);
            if (row.kind.equals("module")) text(ctx, expanded.contains(row.module) ? "-" : "+", hit.x + hit.width - 8, ry + 3, 0xffffffff);
            ...
            if (hover) tooltip = row.setting == null ? row.module.description : row.setting.description;
        }
        ctx.disableScissor();
    ...
    @Override public boolean mouseClicked(Click click, boolean doubled) {
        ...
        switch (hit.kind) {
            case "Meteor" -> { GuiThemes.select("Meteor"); Tabs.get().getFirst().openScreen(GuiThemes.get()); }
            case "HUD" -> {
                var hud = Modules.get().get(PhobosHud.class);
                if (click.button() == 1) mc.setScreen(look.moduleScreen(hud)); else hud.toggle();
            }
            case "Reset" -> arrange(true);
            case "Tabs" -> { var screen = new meteordevelopment.meteorclient.gui.screens.ModulesScreen(look); screen.addDirect(look.topBar()).top().centerX(); mc.setScreen(screen); }
    ...
    @Override public boolean mouseScrolled(double x, double y, double horizontal, double vertical) {
        Hit target = hit(x, y);
        if (target != null && target.panel != null) target.panel.scroll = PhobosPanelLayout.scroll(target.panel.scroll - (int) (vertical * ROW * 2), target.panel.total, target.panel.view);
        return true;
    }
    ...
    @Override public void removed() { dragging = null; sliding = null; GuiKeyEvents.canUseKeys = true; GuiThemes.save(); super.removed(); }
}
