...
public class TitleScreenCredits {
    ...
    private static void init() {
        ...
        MeteorExecutor.execute(() -> {
            for (Credit credit : credits) {
                // This edition has no published BedGang repository to compare against.
                if (credit.addon == MeteorClient.ADDON) continue;
    ...
    public static void render(DrawContext context) {
        ...
        meteordevelopment.meteorclient.utils.render.BedGangBrand.title(context, mc.currentScreen.width);

        int y = mc.currentScreen.width < 540 ? 49 : 3;
        for (Credit credit : credits) {
            synchronized (credit.text) {
    ...
    public static boolean onClicked(double mouseX, double mouseY) {
        ...
        for (Credit credit : credits) {
            ...
            if (mouseX >= x && mouseX <= x + width && mouseY >= y && mouseY <= y + mc.textRenderer.fontHeight + 2) {
                if (credit.addon == MeteorClient.ADDON) return false;
