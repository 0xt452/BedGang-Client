...
public class Hud extends System<Hud> implements Iterable<HudElement> {
    ...
    private boolean shouldHideHud() {
        var impact = meteordevelopment.meteorclient.systems.modules.Modules.get().get(meteordevelopment.meteorclient.systems.modules.impact.ImpactHud.class);
        if (impact != null && impact.replacesMeteorHud()) return true;
        var phobos = meteordevelopment.meteorclient.systems.modules.Modules.get().get(meteordevelopment.meteorclient.systems.modules.phobos.PhobosHud.class);
        if (phobos != null && phobos.replacesMeteorHud() && !HudEditorScreen.isOpen()) return true;
